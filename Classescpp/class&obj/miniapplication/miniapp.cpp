...
class UserManagement{
    ...
    int loginCheck(string name,string PhoneNumber){ //check login info
    ifstream UserFile("Customers.txt",ios::in);
    Customer user;
    while (UserFile >> user)
    {   
        if((user.getName() == name) & (user.getPhoneNumber() == PhoneNumber)){
            return user.getId();
        }
        else{
            continue;
    ...
    bool checkAldreadyExistUsers(string name,string PhoneNumber){ //check if there is already a user with same name and password
    ...
    bool buyProduct(string productType,string productCompanyName,int idofproduct){ //Buy product 
    ...
    bool checkPhoneNumber(string PhoneNumber){ //validate phone number
    ...
    void updateUserFile(vector<Customer> userlist){ //update userfile 
...
class CartManagement{
    ...
    void addProductToCart(Cart &product){ //Adding product to cart
        fstream cartfile("Cart.txt",ios::app);
        cartfile << product;
        cartfile.close();
    }

    void displayCartOfSpecificUser(int userId){ //deleting product from cart
    ...
    bool buy(int userid,int productid,int cartid,vector<Customer> &user){ //buying product from already added porduct in cart 
        fstream cartfile("Cart.txt",ios::in);
        Cart temp;
        ...
        for(int i = 0;i < cartlist.size();i++){
            ...
            if(((cartlist[i].getCustomerId() == userid)&(cartlist[i].getProductId() == productid)) & (cartid == userprocductcount)){
                ...
                manageuser.updateUserFile(user); //updating the user file after buying product
                //deleting the product from the cart after buying
    ...
    vector<Cart> returnSpecificCartList(int userId){ //return vector of product which the user added to the cart
        fstream cartfile("Cart.txt",ios::in);
        Cart temp;
        vector<Cart> cartlist;
        while (cartfile >> temp)
    ...
    bool deleteCart(int userid,int productid,int cartid){ // deleting cart from cart file
        fstream cartfile("Cart.txt",ios::in);
        Cart temp;
        vector<Cart> cartlist;
        while(cartfile >> temp){
        ...
        int deleted = 0;
        for(int i = 0;i < cartlist.size();i++){
            if(cartlist[i].getCustomerId() == userid){ //to keep count of cart product which belong to login user
                userprocductcount++;
            }
            //if the option and our count equal we delete that
            if(((cartlist[i].getCustomerId() == userid)&(cartlist[i].getProductId() == productid)) & (cartid == userprocductcount)){
                deleted = 1;
                continue;
            }
